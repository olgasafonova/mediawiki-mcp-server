...
- **`wiki edit -i` / `--interactive` flag.** Opens the current page content in `$VISUAL` (or `$EDITOR` if `$VISUAL` is unset) and submits the saved buffer as the new content. Replaces the shell idiom `wiki page $P > tmp && vi tmp && wiki edit $P < tmp` (the one @strk reported doing "way too often" in #75). Behavior: empty buffer or unchanged buffer skips the edit; a unified diff of the changes is shown and a `[y/N]` confirmation is required before submitting (declining keeps the buffer); `--dry-run` shows the diff and skips submission, printing a `--file` hint for later submission; submission failure preserves the temp file and prints its path; CAPTCHA retry mirrors the non-interactive flow (max 3 attempts, empty answer gives up). `-i` is mutually exclusive with `--content`, `--file`, piped stdin, `--json`, and `--section` (interactive mode edits the full page). Attaches `/dev/tty` to the child editor so the flow works even when the CLI's own stdin/stdout are piped.
